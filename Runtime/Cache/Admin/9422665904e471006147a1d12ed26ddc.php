...
<head>
    <meta charset="UTF-8">
    <title>海南微软创新中心实习生网站</title>
    <link href="/Public/favicon.png" type="image/x-png" rel="shortcut icon">
...
<body>
    ...
    <div class="main-title">
        <h2>[<?php echo ($model['title']); ?>]最新评论</h2>
    </div>
	<div class="cf">
		<div class="fl">
		<?php if(empty($model["extend"])): ?><div class="tools">
				<!-- <a class="btn" href="<?php echo U('add?model='.$model['id']);?>">新 增</a> -->
				<button class="btn ajax-post confirm" target-form="ids" url="/admin.php?s=/Comment/setStatus/status/-1">删 除</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
            ...
            <table>
                ...
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <?php if(is_array($list_grids)): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$grid): $mod = ($i % 2 );++$i; if($grid['field'][0] == 'status'): if($data['status'] == 1): ?><td class="success">已通过</td>
                                            <?php else: ?>
                                                <td class="fail">已屏蔽</td><?php endif; ?> 
                                        <?php else: ?>
                                        <td><?php echo get_list_field($data,$grid);?></td><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                                <td>
                                    <a href="/admin.php?s=/Comment/setStatus/ids/<?php echo ($data['id']); ?>/status/-1">删除</a>
                                    <?php if($data['status'] == 1): ?>&nbsp&nbsp<a href="/admin.php?s=/Comment/setStatus/ids/<?php echo ($data['id']); ?>/status/0">屏蔽</a>
                                        <?php else: ?>
                                        &nbsp&nbsp<a href="/admin.php?s=/Comment/setStatus/ids/<?php echo ($data['id']); ?>/status/1">通过</a><?php endif; ?> 
                                    &nbsp&nbsp<a href="/admin.php?s=/Comment/viewRes/pid/<?php echo ($data['id']); ?>">查看回复</a>
                                </td>
    ...
    <div class="page">
        ...
        <div class="cont-ft">
            <div class="copyright">
                <div class="fl">感谢使用HIP工作室管理平台</div>
