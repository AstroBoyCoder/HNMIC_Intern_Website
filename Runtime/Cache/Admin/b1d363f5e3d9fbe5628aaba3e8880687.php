...
<body>
	...
	<div class="cf">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
				...
				<button class="btn ajax-post confirm" target-form="ids" url="/onethink/admin.php?s=/Comment/setStatus/status/-1">删 除</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
            ...
            <table>
                ...
                <tbody>
                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
                            <td><input class="ids" type="checkbox" value="<?php echo ($item['id']); ?>" name="ids[]"></td>
                            <td><?php echo ($i); ?></td>
                            <td><?php echo ($item["uid"]); ?></td>
                            <td><?php echo ($item["content"]); ?></td>
                            <td><?php echo ($item["comment_time"]); ?></td>
                            <?php if($item['status'] == 1): ?><td class="success">已通过</td>
                                <?php else: ?>
                                <td class="fail">已屏蔽</td><?php endif; ?>
                            <td>
                                <a href="/onethink/admin.php?s=/Comment/setStatus/ids/<?php echo ($item['id']); ?>/status/-1">删除</a>
                                <?php if($item['status'] == 1): ?>&nbsp&nbsp<a href="/onethink/admin.php?s=/Comment/setStatus/ids/<?php echo ($item['id']); ?>/status/0">屏蔽</a>
                                    <?php else: ?>
                                    &nbsp&nbsp<a href="/onethink/admin.php?s=/Comment/setStatus/ids/<?php echo ($item['id']); ?>/status/1">通过</a><?php endif; ?> 
                            </td>
    ...
    <div class="page">
        ...
        <div class="cont-ft">
            <div class="copyright">
                <div class="fl">感谢使用<a href="#" target="_blank">Hi!Partner</a>管理平台</div>
